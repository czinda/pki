...
This operation is optional for the admin. When the admin creates a new subsystem, a client security database will automatically be created (e.g. `~/.dogtag/pki-quarkus/ca/alias`). The database can be used directly as follows:

----
$ pki -d ~/.dogtag/pki-quarkus/ca/alias -c Secret.123 -n caadmin ca-user-find
----
...
----------------------
  ...
  Subject DN: CN=CA Signing Certificate,OU=pki-quarkus,O=EXAMPLE
  Issuer DN: CN=CA Signing Certificate,OU=pki-quarkus,O=EXAMPLE
