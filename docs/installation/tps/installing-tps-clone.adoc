...
Prepare a deployment configuration, for example `tps-clone.cfg`, to deploy TPS subsystem clone. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
After installation the existing TPS system certificates (including the certificate chain) and their keys are stored in the server NSS database, that is `/var/lib/pki/pki-quarkus/conf/alias`, and a new SSL server certificate is created for the new instance:
