...
Prepare a deployment configuration, for example `ca-secure-ds.cfg`, to deploy CA subsystem. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
After installation the CA system certificates with their keys are generated and stored in the server NSS database, that is `/var/lib/pki/pki-quarkus/conf/alias`, and the DS signing certificate is imported into the same NSS database:
...
After installation the admin certificate and key are stored in `~/.dogtag/pki-quarkus/ca_admin_cert.p12`. The PKCS #12 password is specified in the `pki_client_pkcs12_password` parameter.
