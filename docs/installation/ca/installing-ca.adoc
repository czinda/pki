...
Prepare a deployment configuration, for example `ca.cfg`, to deploy CA subsystem. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
After installation the admin certificate and key are stored in `~/.dogtag/pki-quarkus/ca_admin_cert.p12`.
