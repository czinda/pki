...
[literal]
....
$ pki -d /var/lib/pki/pki-quarkus/conf/alias -f /var/lib/pki/pki-quarkus/conf/password.conf \
...
Prepare a deployment configuration, for example `ca-secure-ds-secondary.cfg`, to deploy a CA subsystem clone. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
After installation the existing CA system certificates (including the certificate chain) and their keys are stored in the server NSS database (i.e. `/var/lib/pki/pki-quarkus/conf/alias`), and a new SSL server certificate is created for the new instance. Note that if the ds_signing certificate is the same certificate as ca_signing, it does not show in the following `certutil` listing:
[literal]
....
