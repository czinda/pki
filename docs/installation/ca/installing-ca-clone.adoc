...
Prepare a deployment configuration, for example `ca-clone-w-p12.cfg`, to deploy a CA subsystem clone. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
and their keys are stored in the server NSS database (i.e. `/var/lib/pki/pki-quarkus/conf/alias`),
