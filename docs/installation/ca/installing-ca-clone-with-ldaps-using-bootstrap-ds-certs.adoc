...
Prepare a deployment configuration, for example `ca-secure-ds-secondary.cfg`, to deploy CA subsystem clone. By default the subsystem is deployed into a Tomcat instance called `pki-quarkus`.
...
After installation, the existing CA system certificates (including the certificate chain) and their keys are stored in the server NSS database, that is, `/var/lib/pki/pki-quarkus/conf/alias`) and a new SSL server certificate is created for the new instance:
[literal]
....
