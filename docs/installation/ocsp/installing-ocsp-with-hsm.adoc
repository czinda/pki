...
It installs a OCSP subsystem in a Tomcat instance (default is pki-quarkus) and creates the following NSS databases:

* server NSS database: /var/lib/pki/pki-quarkus/conf/alias

* admin NSS database: ~/.dogtag/pki-quarkus/ocsp/alias
...
....
$ OCSPClient \
 -d /var/lib/pki/pki-quarkus/conf/alias \
 -h pki.example.com \
 -p 8080 \
 -t /ocsp/ee/ocsp \
 -c ca_signing \
