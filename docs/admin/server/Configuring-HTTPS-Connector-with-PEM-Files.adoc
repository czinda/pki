...
$ openssl req \
    ...
    -days 365 \
    -subj "/CN=$HOSTNAME" \
    -keyout /var/lib/pki/pki-quarkus/conf/sslserver.key \
    -out /var/lib/pki/pki-quarkus/conf/sslserver.crt
$ chown pkiuser:pkiuser /var/lib/pki/pki-quarkus/conf/sslserver.crt
$ chmod 660 /var/lib/pki/pki-quarkus/conf/sslserver.crt
$ chown pkiuser:pkiuser /var/lib/pki/pki-quarkus/conf/sslserver.key
$ chmod 660 /var/lib/pki/pki-quarkus/conf/sslserver.key
----
...
This command will create a `<Connector>` and an `<SSLHost>` in `/var/lib/pki/pki-quarkus/conf/server.xml`.
...
$ pki-server http-connector-cert-add \
    --certFile /var/lib/pki/pki-quarkus/conf/sslserver.crt \
    --keyFile /var/lib/pki/pki-quarkus/conf/sslserver.key
----

This command will create a `<Certificate>` in `/var/lib/pki/pki-quarkus/conf/server.xml`.
