...
----
$ keytool -genkeypair \
    -keystore /var/lib/pki/pki-quarkus/conf/keystore.jks \
    -storepass Secret.123 \
    -alias "sslserver" \
    -dname "CN=$HOSTNAME" \
    -keyalg RSA \
    -keypass Secret.123
$ chown pkiuser:pkiuser /var/lib/pki/pki-quarkus/conf/keystore.jks
$ chmod 660 /var/lib/pki/pki-quarkus/conf/keystore.jks
----
...
$ pki-server http-connector-cert-add \
    --keyAlias sslserver \
    --keystoreFile /var/lib/pki/pki-quarkus/conf/keystore.jks \
