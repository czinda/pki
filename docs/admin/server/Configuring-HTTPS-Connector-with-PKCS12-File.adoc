...
----
$ keytool -genkeypair \
    -keystore /var/lib/pki/pki-quarkus/conf/keystore.p12 \
    -storetype pkcs12 \
    ...
    -keyalg RSA \
    -keypass Secret.123
$ chown pkiuser:pkiuser /var/lib/pki/pki-quarkus/conf/keystore.p12
$ chmod 660 /var/lib/pki/pki-quarkus/conf/keystore.p12
----
...
$ pki-server http-connector-cert-add \
    --keyAlias sslserver \
    --keystoreType pkcs12 \
    --keystoreFile /var/lib/pki/pki-quarkus/conf/keystore.p12 \
