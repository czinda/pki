 ...
 * Takes TPS-enrolled certificates off-hold (unrevokes certificates
 * that were placed on hold). This is the same unrevocation logic
 * as CADoUnrevokeResource but served at the TPS subsystem URL.
 * Legacy URL: /ee/subsystem/ca/doUnrevoke (mapped as ee/ca/doUnrevokeTPS)
 */
@Path("ee/ca/doUnrevokeTPS")
public class CADoUnrevokeTPS {

    private static final Logger logger = LoggerFactory.getLogger(CADoUnrevokeTPS.class);
    ...
    public Response doUnrevoke(@FormParam("serialNumber") String serialNumString) {
        ...
        logger.info("CADoUnrevokeTPS: Processing TPS unrevocation request");
        ...
        RevocationProcessor processor = new RevocationProcessor("CADoUnrevokeTPS", Locale.getDefault());
        ...
        try {
            ...
            logger.error("CADoUnrevokeTPS: Error pre-processing: {}", e.getMessage(), e);
        ...
        try {
            ...
            if (status == RequestStatus.COMPLETE ||
                ...
                // Check LDAP publishing status (TPS requires LDAP publishing)
            ...
            logger.error("CADoUnrevokeTPS: Error processing: {}", e.getMessage(), e);
