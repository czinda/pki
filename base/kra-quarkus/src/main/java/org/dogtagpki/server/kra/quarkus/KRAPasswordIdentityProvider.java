...
/**
 * KRA-specific identity provider for password authentication.
 *
 * Delegates to PKIAuthenticator to authenticate against the
 * PKI internal user database (LDAP-backed).
 */
@ApplicationScoped
public class KRAPasswordIdentityProvider extends PKIPasswordIdentityProvider {

    private static final Logger logger = LoggerFactory.getLogger(KRAPasswordIdentityProvider.class);
    ...
    protected PKIPrincipalCore authenticateByPassword(String username, String password) {
        try {
            ...
            logger.error("KRAPasswordIdentityProvider: Authentication failed for user {}: {}",
